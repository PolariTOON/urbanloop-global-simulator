            ...
            if next_switch.id != step[1] and (next_switch.id is not None):
                ...
                if not switch.defects[next_switch.id][0]: # il n'y a pas d'anomalies pour rester sur la boucle
                    if (next_switch.my_loop.name) not in table_temp :
                            table_temp[next_switch.my_loop.name] = [step[2], step[3]+100, step[4]+[next_switch.id,next_switch.my_loop.name ]]
                            # TODO 100 à remplacer par distance (switch, point suivant)
                            table_to_cover[next_switch.my_loop.name]=[next_switch.id] + table_temp[next_switch.my_loop.name]
                if not switch.defects[next_switch.id][1]: # il n'y a pas d'anomalies dans la boucle aiguillee
                    if (next_switch.switched_loop.name) not in table_temp:
                        table_temp[next_switch.switched_loop.name] = [step[2], step[3]+next_switch.size, step[4]+[next_switch.id, next_switch.switched_loop.name]]
                        table_to_cover[next_switch.switched_loop.name] = [next_switch.id] + table_temp[next_switch.switched_loop.name]
